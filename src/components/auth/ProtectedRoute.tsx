...
export default function ProtectedRoute({ children }: { children: ReactNode }) {
  ...
  // 🛡️ SIGNAL PROTOCOL GUARD (Dev Only)
  if (typeof window !== "undefined") {
    window.__CHECKOUT_AUTH_GUARD_ACTIVE__ = true;
  }

  // 🛡️ STEP 1: RESOLVE LOADING STATE
  if (loading || authState === "loading") {
    return <FullScreenLoader />;
  }

  // 🛡️ STEP 2: BLOCK GUESTS (Redirection handled by useAuth)
  if (authState === "guest") {
    return <FullScreenLoader />; 
  }

  // 🛡️ STEP 3: BLOCK PARTIAL PROFILES (Redirection handled by useAuth)
  if (authState === "onboarding") {
    return <FullScreenLoader />;
  }

  // 🛡️ STEP 4: VERIFY USER OBJECT INTEGRITY
