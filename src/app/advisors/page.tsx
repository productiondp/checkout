...
export default function AdvisorsPage() {
  ...
  const filteredAdvisors = MOCK_ADVISORS.filter(advisor => 
    ...
    <div className="min-h-screen bg-[#FDFDFF] pb-24">
      ...
      <header className="bg-white border-b border-slate-100 pt-12 pb-10 px-6 lg:px-10">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div>
            <h1 className="text-4xl sm:text-5xl font-black text-[#292828] tracking-tight mb-3">Advisors</h1>
            <p className="text-slate-400 font-bold text-base sm:text-lg uppercase tracking-tight">Get guidance from people who’ve done it</p>
          </div>
      ...
      <section className="mt-16 overflow-hidden">
        ...
        <div className="flex overflow-x-auto no-scrollbar gap-6 px-6 lg:px-10 pb-10 max-w-7xl mx-auto">
          {MOCK_ADVISORS.filter(a => a.matchScore > 90).map((advisor) => (
            <div 
              key={advisor.id}
              onClick={() => router.push(`/advisors/${advisor.id}`)}
              className="min-w-[320px] sm:min-w-[400px] bg-white rounded-[2.5rem] sm:rounded-[3.5rem] p-8 sm:p-10 border border-slate-100 shadow-xl hover:shadow-2xl hover:-translate-y-2 transition-all cursor-pointer group relative overflow-hidden flex flex-col h-full"
            >
...
function AdvisorCard({ advisor }: { advisor: Advisor }) {
  ...
  return (
    <div 
      onClick={() => router.push(`/advisors/${advisor.id}`)}
      className="bg-white rounded-[2rem] sm:rounded-[3rem] p-6 sm:p-10 border border-slate-100 shadow-sm hover:shadow-2xl hover:-translate-y-1 transition-all cursor-pointer group flex flex-col md:flex-row items-center gap-6 sm:gap-10"
    >
