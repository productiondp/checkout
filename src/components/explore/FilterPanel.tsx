...
export default function FilterPanel({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-end p-6 pointer-events-none">
       ...
       <div className="relative w-full max-w-md h-full bg-white rounded-lg shadow-4xl pointer-events-auto flex flex-col overflow-hidden animate-in slide-in-from-right-10 duration-500">
          <div className="p-10 border-b border-slate-50 flex items-center justify-between">
             <h2 className="text-2xl font-black text-[#292828] uppercase ">Discovery Filter</h2>
          ...
          <div className="flex-1 overflow-y-auto p-10 space-y-12 no-scrollbar">
             {sections.map((section, idx) => (
               <div key={idx} className="space-y-6">
                  <h3 className="text-[10px] font-black uppercase  text-slate-400">{section.title}</h3>
                     ...
                     {section.options.map(opt => {
                        ...
                        return (
                          <button 
                            ...
                            className={cn(
                              "px-6 h-12 rounded-lg text-[11px] font-black uppercase  transition-all flex items-center gap-2 border",
                              isActive 
             ...
             <div className="space-y-6">
                <h3 className="text-[10px] font-black uppercase  text-slate-400">Distance Radius</h3>
                <input 
                  type="range" 
          ...
          <div className="p-10 bg-slate-50/50 border-t border-slate-100 grid grid-cols-2 gap-4">
             <button className="h-16 bg-white border border-slate-100 text-slate-400 rounded-lg font-black text-xs uppercase  hover:text-[#292828] transition-all">Reset</button>
             <button onClick={onClose} className="h-16 bg-[#E53935] text-white rounded-lg font-black text-xs uppercase  shadow-xl shadow-red-500/20 active:scale-95 transition-all">Apply Filter</button>
          </div>
       </div>
    </div>
  );
